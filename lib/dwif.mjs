...
export function getDefaultOutputName(inputPath) {
  ...
  if (parsed.ext.toLowerCase() === ".avif") {
    return `${parsed.name}-resized.webp`;
  }
...
async function processExperimentalAvif({
  ...
  const finalOutputPath = isAvifPath(outputPath)
    ? path.join(path.dirname(outputPath), `${path.parse(outputPath).name}.webp`)
    : outputPath;
    ...
    return {
      ...
      warning: [
        ...
        (isAvifPath(inputPath) || isAvifPath(outputPath))
          ? `Experimental AVIF support used a temporary lossless WEBP bridge and exported WEBP instead of ${path.extname(requestedOutputPath) || ".avif"}.`
          : null
