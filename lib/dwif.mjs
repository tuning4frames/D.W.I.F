...
async function writeAnimatedGifWithGifski(
  ...
  try {
    ...
    // A path argument for every generated frame can exceed the OS command-line
    // limit and make spawn() fail with ENAMETOOLONG. Use the in-process encoder
    // when the gifski invocation would be too large.
    const argumentBytes = framePaths.reduce(
      (total, framePath) => total + Buffer.byteLength(framePath) + 1,
      0
    );
